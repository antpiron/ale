...
main(int argc, char *argv[argc])
{
  ...
  ERROR_UNDEF_FATAL_FMT(1048576 != rir->count,
			"FATAL: netutils_rir_getinfo(2.0.0.0) %"PRIu32" != 1048576\n", rir->count);
  ERROR_UNDEF_FATAL_FMT(0 != strcmp("FR",rir->country),
			"FATAL: netutils_rir_getinfo(2.0.0.0) %s != FR\n", rir->country);
  // TODO: Mmmh unable to get epoch consistently
  // posix suXor http://www.catb.org/esr/time-programming/
  ERROR_UNDEF_FATAL_FMT(llabs(1278885600ll - rir->date) > 7200,
			"FATAL: netutils_rir_getinfo(2.0.0.0) %lld != 1278885600\n", (long long) rir->date);
  netutils_destroy(&nu);
